# Parts — YouTube_Translator

| # | Part | Код | Карточка | Статус |
|---|------|-----|----------|--------|
| 01 | [AI sidebar UX](01-ai-sidebar-ux.md) | `Subtitle_App.py` | №2 / D | ✅ |
| 02 | [DeepSeek analyze](02-ai-analyze-deepseek.md) | `ai_analyze.py` | C | ✅ |
| 03 | [Launch & deps](03-launch-and-deps.md) | `launch*`, `requirements.txt` | A | ✅ |
| 04 | [Theater / fullscreen](04-player-theater-fullscreen.md) | `Subtitle_App.py` | №3 / D5 | ✅ |
| 05 | [Bookmarks + Cloudflare](05-bookmarks-vision.md) | `Subtitle_App.py`, `bookmarks.json` | №4 / F | ✅ |
| 06 | [Button motion](06-button-motion-vision.md) | `ui_motion.py` | №8 / G | ✅ |
| 07 | [**ИИ текущего ролика**](07-player-ai-current-video.md) | `Subtitle_App.py`, `ai_analyze.py` | **№1** | ✅ |
| 08 | [Split 📥 / ✨ + Отмена](08-split-download-ai-cancel.md) | `Subtitle_App.py` | №6 | ✅ |
| 09 | [Window size / Snap](09-window-size-snap.md) | `Subtitle_App.py` | №5 / W | ✅ |
| 10 | [Music MP3](10-music-download.md) | `Subtitle_App.py` | №7 / M | ✅ |
| 11 | [Parked 021 / статус 022](11-parked-021-022.md) | — | №9–10 | 021 ⬜ · 022 → 12 |
| 12 | [Overlay музыка/mp4](12-overlay-music.md) | `overlay_player.py` | №10 / IDEA-022 | 🟡 |
| 16 | [Overlay UX polish P1–P9](16-overlay-ux-polish.md) | `overlay_player.py`, `Subtitle_App.py` | №13–18 | ✅ |
| 17 | [UI backup / запас интерфейса](17-ui-backup.md) | `backups/ui/`, `overlay_player.py` | **№19 / U** | 🟡 |

[TZ](../TZ.md) · [PROMPT](../PROMPT_FOR_AGENT.md) · [карта docs](../README.md)
