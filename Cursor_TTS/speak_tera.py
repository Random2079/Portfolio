...
def synthesize_wav(
    ...
    *,
    ...
    del lang  # один RU-голос; EN только через словарь в text_prep, не через <en>
    text = (text or "").strip()
    if len(text) < 1:
        raise ValueError("empty text")
    # Смена <en> меняет тембр и даёт фризы — всегда русский тег.
    text = text.replace("<en>", "<ru>").replace("</en>", "</ru>")
    if "<ru>" not in text:
        text = f"<ru>{text}</ru>"
