...
- `Ctrl + Shift + P` — **пауза / продолжить** (очередь не сбрасывается; продолжение со следующего куска).
- `Ctrl + Shift + X` — полный **стоп**: речь и очередь.
...
Это **не** то же самое, что кнопка «Пауза» / Ctrl+Shift+P (останов воспроизведения с resume).
