...
В окне: авто, **движок** (по умолчанию **Tera**), голос, громкость, пауза, **Tera темп**, «Прослушать», «Пауза», «Стоп».  
Движки: **Tera** · **Edge** · **Silero** · Qwen · OpenAI (опц.). Kokoro убран.  
ТЗ и контракты Stop/warmup/тесты: [`docs/TZ.md`](docs/TZ.md).  
При открытии панели демон поднимается сам и **прогревает** текущий движок в фоне.  
Живой статус показывает: **загрузка модели / синтез N из M / играет N из M / секунды / очередь**.  
Без фейковых %: бегущая полоска на синтезе; при play нескольких кусков — шкала `N / M кусков`.  
На **паузе таймер секунд замирает**. Смена движка во время озвучки/паузы → стоп текущей фразы.  
Stop **не убивает** тёплый Kokoro-worker, если уже играет wav (иначе каждый Стоп = cold load 20–60 с). Worker режется только на synth.  
**Закрытие панели (крестик)** гасит демон + kokoro + зависшие `--warmup` + `hotkey_tts.ahk` — чтобы после рестарта Cursor не висели сироты.  
Опрос панели: idle ~1 с, busy ~0.4 с; без `tasklist`.  
Пишет в те же файлы, что хук и AHK (`TTS_OFF`, `tts_config.json`).

По умолчанию **очередь** (`interrupt_on_new=false`): новый ответ не рвёт текущий synth. Галка «обрывать» — только если сам включишь.
