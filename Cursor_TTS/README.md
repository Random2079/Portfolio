...
На одном куске полоска **бегущая** (модель не отдаёт % внутри синтеза) + секунды; % — когда кусков много или уже играет.  
После Стопа Kokoro-worker убивается (иначе Stop ждал бы synth) и **сразу греется снова в фоне**, чтобы следующий Speak не ловил cold load 15–60 с.  
