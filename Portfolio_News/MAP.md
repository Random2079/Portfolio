...
| **React UI** | `react_Portfolio_News/` Vite+React, референс = ванильный дашборд | ▶️ **R0** |
...
1. **React UI** — папка `../react_Portfolio_News/` · слой **R0** (шелл + прокси к API) · см. §7R  
2. Дальше React-слои по MVP утра (§7R): R1 день+позиции → R2 разбор/график/KS → …  
3. Ваниль G / чат / H — **пауза** (бэкап: `Portfolio_News` + `dashboard-demo.html` на `/`)  
4. Сделать React default вместо ванили — только отдельным решением  

Сейчас фокус: **R0** в `react_Portfolio_News/`.
...
| Слой | Статус | Что |
|------|--------|-----|
| **R0** | ▶️ | Vite+React шелл, прокси `/api` → 8765, тема+motion, health + day KPI (референс ваниль) |
