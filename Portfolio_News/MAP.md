...
**Пласт в git:** KS `f7e6aaf` + F-A `ce299cd` (без пуша).  
**Следующий код:** слой F, этап **B** — one-shot разбор тикера (панель, не чат).
...
Не делаем «два плана сразу». Сейчас очередь: **F-B → чат/KV когда скажешь**.
...
| Новости | `poll_job.py`, `db.NewsItem`, `/api/news`, `ai_noise.py`, `news_ai_cache` |
