...
| Слой | Статус | Описание |
|------|--------|----------|
| K0 | 🟡 | Токен в `.env` → живые позиции; список тикеров UI ← **только портфель БКС** (режим «Все в базе» снят 2026-09-10) |
...
| 7 | **Демо Snowball** без замены главной: `/demo` (`dashboard-demo.html`); бэкап UI → `static/backups/…` и `/legacy`. Лента в демо отдельным блоком «ситуация рынка». **Одобрено пользователем 2026-09-10** («гораздо лучше»). След. шаг по команде: сделать `/demo` дефолтом (`/`), старый UI оставить на `/legacy` |
