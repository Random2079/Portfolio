...
| F | ✅ | ИИ: **A** чистка ленты + **B** разбор тикера ([MAP](../MAP.md)); чат позже |
| G | ⬜ | Авто-watch — план в [MAP §7G](../MAP.md) (Scheduler → `once`) |
| H | ⬜ | Телефон (ntfy…) |
...
- [x] **KS** сверка / факты под карточкой (чек-поинт UI, без «купи»)
...
| K8 | `/api/operations` + фильтры + `/api/operations.csv` |
| K9 | `once --notify digest` на холодную → toast только по сегодняшним; `off` = тишина; UI Новости → Toast |
| KS | разбор MTSS/TATNP → позиция + «Сверка»; облигация → ISIN/купон/класс; нигде нет «купи» |
...
| [10-bcs-terminal](parts/10-bcs-terminal.md) | история, графики, разбор K0–K9 |
| [11-review-checkpoint](parts/11-review-checkpoint.md) | **KS** — сверка / факты в разборе |
...
| 3 | Новости: мусор? → на что бьёт? → срочно? → **чеклист к стратегии**, не ордер (слой F: A лента → B разбор; чат позже; без «купи/продай») |
...
Приоритет кода: **K0 → … → K9 ✅**, затем слой F; **KV** — низкий приоритет; IDEA-023 forecast — не трогать без команды.